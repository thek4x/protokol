...
(function (jQuery) {
    ...
    if (location.href.search('details') >= 0) {
        $(".tab-pane").find('p').each(function (el, index) {
            if ($(index).text() == '') {
                $(index).remove()
            }
        });

        $("#drugDetails").find('table').each(function (index, el) {
            $(el).addClass('table table-striped table-bordered mt-3 border-0');
        });
    }
})(jQuery);